...
import { deleteNumberConfig } from "@/lib/simwood";
...
export async function DELETE(
  ...
  try {
    ...
    await deleteNumberConfig(number);

    await createAuditLog(
      AUDIT_LOG_ACTIONS.SIMWOOD,
      "SIMWOOD",
      requestId,
      { action: "reset-number-config", number }
    );

    return NextResponse.json({ success: true, message: "Number configuration reset" });
  } catch (error) {
    console.error("Error resetting number config:", error);
    return NextResponse.json(
